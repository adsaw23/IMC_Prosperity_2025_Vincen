from data_model import OrderDepth, UserId, TradingState, Order
...
class Trader:
    ...
    def resin_orders(self, order_depth: OrderDepth, fair_value: int, position: int, limit: int) -> List[Order]:
        ...
        if new_position > two_third_limit:
    ...
    def squid_orders(self, order_depth: OrderDepth, timespan: int, width: float, squid_take_width: float, position: int, position_limit: int) -> List[Order]:
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            ...
            # Avoid trading in unstable markets
            if spread > 3 or spread < 2:
                return orders

            # Use only strong volume levels to avoid noise
            ...
            # Estimate fair value with volatility buffer
            recent_max = max(self.squid_prices)
            recent_min = min(self.squid_prices)
            volatility = recent_max - recent_min
            base_fair_value = (recent_max + recent_min) / 2
            ...
            # Adjust fair value
            fair_value = base_fair_value + (imbalance * volatility * 0.5)

            # Market taking logic - aggressive if prices are favorable
            if fair_value <= best_bid <= fair_value + squid_take_width:
            ...
            # Market making logic - passive placement around adjusted fair value
            aaf = [price for price in order_depth.sell_orders.keys() if price > fair_value + 1]
            bbf = [price for price in order_depth.buy_orders.keys() if price < fair_value - 1]
            baaf = min(aaf) if len(aaf) > 0 else fair_value + 2
            bbbf = max(bbf) if len(bbf) > 0 else fair_value - 2
