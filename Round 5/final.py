...
def basket2_correlation_strategy(state: TradingState, data: Dict[str, Any],
                               basket_symbol: str = "PICNIC_BASKET2",
                               croissant_symbol: str = "CROISSANTS",
                               jam_symbol: str = "JAMS",
                               basket_limit: int = 100,
                               croissant_limit: int = 250,
                               jam_limit: int = 350) -> Tuple[Dict[str, List[Order]]]:
    """
    Basket trading strategy using correlation between components
    Returns orders for basket, croissants, and jams
    """
    # Initialize orders
    orders = {
        basket_symbol: [],
    ...
    # Initialize spread window (would normally persist between rounds)
    ...
    def get_composite_value(state):
        """Calculate theoretical basket value based on components"""
        croissant_price = get_mid_price(state.order_depths.get(croissant_symbol, OrderDepth()))
        jam_price = get_mid_price(state.order_depths.get(jam_symbol, OrderDepth()))
        if croissant_price is None or jam_price is None:
            return None
        return 2 * croissant_price + 4 * jam_price

    def get_spread(state):
        """Calculate spread between basket and components"""
        basket_price = get_mid_price(state.order_depths.get(basket_symbol, OrderDepth()))
        composite_value = get_composite_value(state)
        if basket_price is None or composite_value is None:
            return None
        return basket_price - composite_value

    # Main trading logic
    spread = get_spread(state)
    if spread is not None:
        data['spread_window'].append(spread)
    
    # Calculate z-score if we have enough data
    z_score = 0
    if len(data['spread_window']) >= 10:
        spread_mean = np.mean(data['spread_window'])
        spread_std = np.std(data['spread_window'])
        if spread_std > 0:
            z_score = (spread - spread_mean) / spread_std
    if len(data['spread_window']) >= 15:
        data['spread_window'].pop(0)

    # Get current positions
    basket_pos = state.position.get(basket_symbol, 0)
    croissant_pos = state.position.get(croissant_symbol, 0)
    jam_pos = state.position.get(jam_symbol, 0)

    # Trading logic based on spread z-score
    if z_score > 1.0:  # Basket overpriced
        # Sell basket and buy components
        basket_price = get_mid_price(state.order_depths[basket_symbol])
        if basket_price is not None:
            available = basket_limit + basket_pos
            if available > 0:
                price = basket_price + 1
                quantity = min(available, 5)
                orders[basket_symbol].append(Order(basket_symbol,int(price), -quantity))
        
        # Hedge components (buy)
        croissant_price = get_mid_price(state.order_depths[croissant_symbol])
        jam_price = get_mid_price(state.order_depths[jam_symbol])
        
        if croissant_price is not None:
            croissant_qty = int(2 * hedge_ratio * correlation_factors['croissant'])
            best_ask = min(state.order_depths[croissant_symbol].sell_orders.keys())
            quantity = min(-state.order_depths[croissant_symbol].sell_orders[best_ask], 
                          croissant_qty, 
                          croissant_limit - croissant_pos)
            if quantity > 0:
                orders[croissant_symbol].append(Order(croissant_symbol, best_ask, quantity))
        
        if jam_price is not None:
            jam_qty = int(4 * hedge_ratio * correlation_factors['jam'])
            best_ask = min(state.order_depths[jam_symbol].sell_orders.keys())
            quantity = min(-state.order_depths[jam_symbol].sell_orders[best_ask], 
                          jam_qty, 
                          jam_limit - jam_pos)
            if quantity > 0:
                orders[jam_symbol].append(Order(jam_symbol, best_ask, quantity))

    elif z_score < -1.0:  # Basket underpriced
        ...
        # Hedge components (sell)
        croissant_price = get_mid_price(state.order_depths[croissant_symbol])
        jam_price = get_mid_price(state.order_depths[jam_symbol])
        
        if croissant_price is not None:
            croissant_qty = int(2 * hedge_ratio * correlation_factors['croissant'])
            best_bid = max(state.order_depths[croissant_symbol].buy_orders.keys())
            quantity = min(state.order_depths[croissant_symbol].buy_orders[best_bid], 
                          croissant_qty, 
                          croissant_limit + croissant_pos)
            if quantity > 0:
                orders[croissant_symbol].append(Order(croissant_symbol, best_bid, -quantity))
        
        if jam_price is not None:
            jam_qty = int(4 * hedge_ratio * correlation_factors['jam'])
            best_bid = max(state.order_depths[jam_symbol].buy_orders.keys())
            quantity = min(state.order_depths[jam_symbol].buy_orders[best_bid], 
                          jam_qty, 
                          jam_limit + jam_pos)
            if quantity > 0:
                orders[jam_symbol].append(Order(jam_symbol, best_bid, -quantity))

    else:  # Neutral zone - market make
        composite_value = get_composite_value(state)
        if composite_value is not None:
            # Market make basket
            orders[basket_symbol].append(Order(basket_symbol, int(composite_value - 1), 3))
            orders[basket_symbol].append(Order(basket_symbol, int(composite_value + 1), -3))
        
        # Market make components
        croissant_price = get_mid_price(state.order_depths[croissant_symbol])
        if croissant_price is not None:
            orders[croissant_symbol].append(Order(croissant_symbol, int(croissant_price - 1), 2))
            orders[croissant_symbol].append(Order(croissant_symbol, int(croissant_price + 1), -2))
        
        jam_price = get_mid_price(state.order_depths[jam_symbol])
        if jam_price is not None:
            orders[jam_symbol].append(Order(jam_symbol, int(jam_price - 1), 1))
            orders[jam_symbol].append(Order(jam_symbol, int(jam_price + 1), -1))

    return orders[basket_symbol], orders[jam_symbol], orders[croissant_symbol]
...
class Trader:
    def __init__(self, params=None):
        ...
        limits = {
            ...
            "PICNIC_BASKET2": 100,
            # "VOLCANIC_ROCK" : 400,
            # "VOLCANIC_ROCK_VOUCHER_9500" :  200, #200
            # "VOLCANIC_ROCK_VOUCHER_9750" :  200, #200
            # "VOLCANIC_ROCK_VOUCHER_10000" : 200, #200
            # "VOLCANIC_ROCK_VOUCHER_10250" : 200, #200
            # "VOLCANIC_ROCK_VOUCHER_10500" : 200, #200
    ...
    def jam_orders(self, order_depth: OrderDepth, position: int, data: Dict[str, Any] ) -> List[Order]:
        orders: List[Order] = []
        if order_depth.buy_orders and order_depth.sell_orders:
            ...
            data["mid_price_history"].append(mid_price)
            ...
            if len(data["mid_price_history"]) > self.window_size:
                fair_value = np.mean(data["mid_price_history"][-self.window_size:])
            else:
            ...
            position_factor = 1 - abs(position) /350
            base_size = 25
            dynamic_size = int(base_size * position_factor)
            
            if position < 350:
                orders.append(Order("JAMS", bid_price, dynamic_size))
            if position > -350:
                orders.append(Order("JAMS", ask_price, -dynamic_size))
            ...
            data["imbalance_history"].append(imbalance)
            
            if len(data["imbalance_history"]) > self.window_size:
                avg_imbalance = np.mean(data["imbalance_history"][-self.window_size:])
            else:
                avg_imbalance = imbalance
            
            if imbalance > max(100, avg_imbalance * 1.5):  # Strong buy signal
                aggressive_buy_price = best_ask
                buy_size = min(30, 350 - position)
                if buy_size > 0:
                    orders.append(Order("JAMS", aggressive_buy_price, buy_size))
            
            elif imbalance < min(-100, avg_imbalance * 1.5):  # Strong sell signal
                aggressive_sell_price = best_bid
                sell_size = min(30, 350 + position)
                if sell_size > 0:
                    orders.append(Order("JAMS", aggressive_sell_price, -sell_size))
            ...
            if self.highest_price_seen > self.lowest_price_seen:
                ...
                if mid_price <= lower_threshold and position < 350:
                    orders.append(Order("JAMS", best_bid + 1, min(40, 350 - position)))
                
                elif mid_price >= upper_threshold and position > -350:
                    orders.append(Order("JAMS", best_ask - 1, -min(40, 350 + position)))
        
        if len(data["spread_history"]) > self.window_size:
            data["spread_history"].pop(0)
        if len(data["mid_price_history"]) > self.window_size:
            data["mid_price_history"].pop(0)
        if len(data["imbalance_history"]) > self.window_size:
            data["imbalance_history"].pop(0)
        
        if len(data["position_history"]) > self.window_size:
            data["position_history"].pop(0)
    
        data["position_history"].append(position)
    ...
    # )
        ...
        if Product.PICNIC_BASKET2 in state.order_depths:
            basket_orders, jams_orders, croissant_orders = basket2_correlation_strategy(state, traderObject[Product.SPREAD2])
        ...
        #     if symbol in traderObject:
        #         strategy.load(traderObject[symbol])
        ...
        #         jam_orders = self.jam_orders(state.order_depths[Product.JAMS],
        #                                         jam_position, 
        #                                         traderObject[Product.JAMS])
